...
void ov_ds_service_t::service()
{
  ...
  wsclient.set_message_handler([&](websocket_incoming_message ret_msg) {
    ...
    if(ret_str == "hey") {
      ucout << "ret_str " << utility::conversions::to_string_t(ret_str) << "\n";
    } else {
      try {
        ...
        if(j["data"][0] == "user-ready") {
          ucout << "\n/--  USER_READY --/\n" << std::endl;
          ucout << j["data"][1].dump(4).c_str() << std::endl;
          ...
          ucout << "_id:  " << ((std::string) user["_id"]).c_str() << std::endl;

          // or we can dump nlohmann::json objects like this:
          ucout << "user:  " << user.dump(4).c_str() << std::endl;
        }
        ...
        if(j["data"][0] == "sound-card-added") {
          ...
          ucout << "sound_cards:\n" << sound_cards.dump(4).c_str() << std::endl;
        }
        // ------------------------------
        // -----    device-added    -----
        // ------------------------------
        ...
        if(j["data"] == "stage-member-added") {
          ...
          ucout << "stage-members:\n"
                << stage_members.dump(4).c_str() << std::endl;
        }
        ...
        if(j["data"][0] == "track-preset-added") {
          ...
          // print groups jsonObject array
          ucout << "track-presets:\n"
                << track_presets.dump(4).c_str() << std::endl;
        }
      }
  ...
  ucout << "Print device json string: \n" << deviceJson.dump().c_str()
        << std::endl;
  ucout << "PrettyPrint device json: \n" << deviceJson.dump(4).c_str()
        << std::endl;

  std::string body_str("{\"type\":0,\"data\":[\"token\",{\"token\":\"" +
                       this->token_ + "\"}]}");

  ucout << "token / device msg: " << body_str.c_str() << std::endl;
