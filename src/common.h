...
inline secret_t &msg_secret(char *m) { return *((secret_t *)m); };
inline stage_device_id_t &msg_callerid(char *m) {
  return *((stage_device_id_t *)(&m[POS_CALLERID]));
};
inline port_t &msg_port(char *m) { return *((port_t *)(&m[POS_PORT])); };
inline sequence_t &msg_seq(char *m) { return *((sequence_t *)(&m[POS_SEQ])); };
